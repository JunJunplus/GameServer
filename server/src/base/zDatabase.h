...
struct zSetObjectB:public zEntry
{
	...
	{
		//soke 灵魂锁链  这地方不应该清除  否则sets里面就一个数据 本来应该5个
		//sets.clear();
...
//------------------------------------
// soke   紫装PurpleObjectBase
//------------------------------------
struct PurpleObjectBase{
	const DWORD getUniqueID() const
	{
		return dwField0;
	}
...
struct zPurpleObjectB:public zEntry
{
	WORD	holy;				// 神圣一击
	luckRangeValue	damage;		// 增加伤害值x％
	luckRangeValue	fivepoint;		// 五行属性增加
	...
	void fill(PurpleObjectBase &data)
	{
		id = data.dwField0;
		strncpy(name, data.strField1, MAX_NAMESIZE);
		holy = atoi(data.strField2);
